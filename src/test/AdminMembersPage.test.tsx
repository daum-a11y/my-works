...
describe('AdminMembersPage', () => {
  it('renders the original-style member table without auth queue columns', async () => {
    ...
    expect(
      screen.getByRole('columnheader', { name: /ID 오름차순으로 정렬 중/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /이름 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /이메일 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /권한 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /활성여부 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /승인상태 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /업무보고접근 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /등록일 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: /최종로그인 정렬 기준 선택 가능/ }),
    ).toBeInTheDocument();
