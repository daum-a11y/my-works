...
describe("UserProfilePage", () => {
  ...
  const getNextPasswordInput = () => screen.getByLabelText("새 비밀번호", { selector: "input" }) as HTMLInputElement;
  ...
  it("shows confirm and completion states, then returns to login with a notice", async () => {
    ...
    expect(screen.getByText("비밀번호를 정말 변경하시겠습니까? 되돌릴 수 없습니다.")).toBeInTheDocument();
    ...
    expect(await screen.findByText("비밀번호가 변경되었습니다. 로그인해 주세요.")).toBeInTheDocument();
