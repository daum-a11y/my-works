...
function toServiceGroups(items: Awaited<ReturnType<typeof adminDataClient.listServiceGroups>>): ServiceGroup[] {
  return items.map((item) => ({
    id: item.id,
...
function toPages(items: Awaited<ReturnType<typeof adminDataClient.listProjectPages>>): ProjectPage[] {
  return items.map((item) => ({
    id: item.id,
    legacyPageId: "",
...
export function AdminReportEditorPage() {
  ...
  const serviceGroups = useMemo(() => toServiceGroups(serviceGroupsQuery.data ?? []), [serviceGroupsQuery.data]);
  ...
  const projectsById = useMemo(() => new Map(projects.map((item) => [item.id, item] as const)), [projects]);
  const pagesById = useMemo(() => new Map(pages.map((item) => [item.id, item] as const)), [pages]);
  const serviceGroupsById = useMemo(() => new Map(serviceGroups.map((item) => [item.id, item] as const)), [serviceGroups]);
  ...
  const projectOptions = useMemo(() => buildProjectViewModels(projects, serviceGroups), [projects, serviceGroups]);
  ...
  const draftPages = useMemo(() => pages.filter((page) => page.projectId === draft.projectId), [draft.projectId, pages]);
  ...
  const selectedType2LegacyId = useMemo(
    () => taskTypes.find((taskType) => taskType.type1 === draft.type1 && taskType.type2 === draft.type2)?.legacyTypeId ?? "",
    [draft.type1, draft.type2, taskTypes],
  );
  const reportTabType1Options = useMemo(() => {
    const legacyOrder = ["민원", "데이터버퍼", "일반버퍼", "교육", "기타버퍼", "휴무"];
    const available = legacyOrder.filter((type1) => buildTaskType1Options(taskTypes).includes(type1));
  ...
  const type2Options = useMemo(() => buildTaskType2Options(taskTypes, draft.type1), [draft.type1, taskTypes]);
          ...
          <form className={styles.form} onSubmit={onSubmit}>
            ...
            {activeTab === "report" ? (
              <div className={styles.formGrid}>
                ...
                <div className={styles.searchButtonField}>
                  ...
                  <button type="button" className={styles.secondaryButton} onClick={() => setAppliedProjectQuery(projectQuery)}>
                    검색
                  </button>
                </div>
            ...
            <div className={styles.formGrid}>
              {projectTypeSelected ? (
                ...
                <label className={styles.field}>
                  <span>타입1</span>
                  <select value={draft.type1} onChange={(event) => setDraftField("type1", event.target.value)}>
              ...
              {showProjectSelect && !isProjectLinkedTab ? (
                <label className={styles.field}>
                  <span>프로젝트</span>
                  <select value={draft.projectId} onChange={(event) => setDraftField("projectId", event.target.value)}>
                    <option value="">{typeFilteredProjects.length ? "선택하세요" : "프로젝트가 존재하지 않습니다."}</option>
              ...
              {showPageSelect ? (
                <label className={styles.field}>
                  ...
                  <select value={draft.pageId} onChange={(event) => setDraftField("pageId", event.target.value)}>
                    <option value="">{draftPages.length ? "선택하세요" : "페이지가 존재하지 않습니다."}</option>
                    {draftPages.map((page) => (
            ...
            <label className={styles.field}>
              <span>비고</span>
              <textarea value={draft.note} onChange={(event) => setDraftField("note", event.target.value)} rows={2} />
            </label>

            {isEdit && currentMember ? <p className={styles.status}>사용자: {currentMember.legacyUserId} ({currentMember.name})</p> : null}

            <div className={styles.actionRow}>
              <button type="submit" className={styles.primaryButton} disabled={saveMutation.isPending}>
                저장
              </button>
              <button type="button" className={styles.secondaryButton} onClick={() => navigate("/admin/reports")}>
                취소
              </button>
              <button type="button" className={styles.secondaryButton} onClick={() => setDraftField("reportDate", shiftDateInput(draft.reportDate || getTodayInputValue(), -1))}>
                이전일
              </button>
              <button type="button" className={styles.secondaryButton} onClick={() => setDraftField("reportDate", getTodayInputValue())}>
                오늘
              </button>
              <button type="button" className={styles.secondaryButton} onClick={() => setDraftField("reportDate", shiftDateInput(draft.reportDate || getTodayInputValue(), 1))}>
                다음일
