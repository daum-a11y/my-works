...
export function SearchPage() {
  ...
  const handleSaveAdd = async () => {
    await saveMutation.mutateAsync({
      ...
      taskType1: addDraft.type1,
      taskType2: addDraft.type2,
      hours: Number.parseFloat(addDraft.workHours) || 0,
  ...
  const handleSaveEdit = async () => {
    ...
    await saveMutation.mutateAsync({
      ...
      taskType1: editingDraft.type1,
      taskType2: editingDraft.type2,
      hours: Number.parseFloat(editingDraft.workHours) || 0,
    ...
    <section className={styles.page}>
              ...
              {sortedReports.map((report) => {
                  ...
                  return (
                    <tr key={report.id} data-active>
                      ...
                      <td className={styles.rowActions}>
                        <button type="button" className={styles.primaryButton} onClick={() => void handleSaveEdit()}>
                          저장
                        </button>
                        <button type="button" className={styles.secondaryButton} onClick={cancelEdit}>
                          취소
                        </button>
                        <button type="button" className={styles.ghostButton} onClick={() => void handleDelete(report)}>
                          삭제
                        </button>
                      </td>
                    </tr>
                ...
                return (
                  <tr key={report.id}>
                    ...
                    <td>{report.note}</td>
                    <td className={styles.rowActions}>
                      <button type="button" className={styles.secondaryButton} onClick={() => beginEdit(report)}>
                        수정
                      </button>
                      <button type="button" className={styles.ghostButton} onClick={() => void handleDelete(report)}>
                        삭제
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.actionRow}>
          <button type="button" className={styles.primaryButton} onClick={() => void handleSaveAdd()}>
            추가
          </button>
          <button type="button" className={styles.secondaryButton} onClick={() => setAddDraft(createEmptyReportDraft())}>
            다시 작성
