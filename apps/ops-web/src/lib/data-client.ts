...
function createSupabaseClient(): OpsDataClient {
  ...
  return {
    ...
    async getProjectPages(member) {
      let query = supabase.from("project_pages").select("*").order("updated_at", { ascending: false });
    ...
    async getAllProjectPages() {
      const { data, error } = await supabase.from("project_pages").select("*").order("updated_at", { ascending: false });
      if (error) throw error;
      return (data ?? []).map(mapProjectPageRecord);
    },
    ...
    async getDashboard() {
      const [{ data: pages, error: pagesError }, { data: projects, error: projectError }, { data: members, error: membersError }] =
        await Promise.all([
          supabase.from("project_pages").select("*"),
