...
8. 이미 운영 중인 DB라면 [005_harden_auth_rls.sql](/Volumes/workspace/workspace/my-works/supabase/sql/005_harden_auth_rls.sql) 추가 적용
9. 승인대기 상태를 운영할 경우 [006_member_status_pending.sql](/Volumes/workspace/workspace/my-works/supabase/sql/006_member_status_pending.sql) 추가 적용
10. 관리자 초대 메일 기능을 쓸 경우 `invite-member` 함수 배포 및 설정
...
Supabase Dashboard -> Authentication -> Providers -> Email 에서 일반 이메일 회원가입 옵션을 끈다.

- 이 앱은 공개 회원가입을 지원하지 않는다.
- 계정 생성은 관리자 초대 기준으로만 운영한다.
- 이 설정이 열려 있으면 초대되지 않은 사용자가 Auth 계정을 만드는 경로가 생긴다.
