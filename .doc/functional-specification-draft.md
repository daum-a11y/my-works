...
| 업무보고 | `/person/report` | 업무보고 | REPORT-005 | 업무 타입 선택 | 업무 타입1 선택 시 타입2 후보를 필터링하고, `src/utils/taskType.ts` 규칙에 따라 서비스/프로젝트 입력 필요 여부를 결정한다. | 타입1/타입2 기준정보, 활성 여부. | 타입2 옵션 갱신, 종속 필드 표시/검증. | `src/pages/reports/ReportsEditorForm.tsx`, `src/utils/taskType.ts` | 구현됨 | 구현됨 | 타입1 변경 시 타입2 초기화, 비활성 타입 제외 확인 |
...
| 프로젝트 | `/projects/:projectId/edit` | 프로젝트 편집 | PROJECT-EDIT-002 | 프로젝트 삭제 | 기존 프로젝트를 삭제한다. 삭제 API 실패 시 오류를 표시하고 삭제 상태로 처리하지 않는다. | 프로젝트 ID 필수. | 삭제 후 `/projects` 이동, 실패 시 오류. | `src/pages/projects/ProjectEditorPage.tsx`, `src/api/projects` | 구현됨 | 구현됨 | 삭제 성공/실패 확인 |
| 프로젝트 | 프로젝트 편집 | 페이지/태스크 관리 | PROJECT-PAGE-001 | 페이지 추가 | 프로젝트 하위 페이지 행을 추가하고 이름, URL, 타입, 추적 상태 등을 입력한다. | 페이지명 필수, URL은 입력값 그대로 저장 대상. | 행 추가, 저장 시 신규 페이지 생성. | `src/pages/projects/ProjectEditorPage.tsx`, `src/pages/projects/ProjectEditorActionRow.tsx` | 구현됨 | 구현됨 | 행 추가, 필수값 누락, 저장 반영 확인 |
...
| 프로젝트 | 프로젝트 편집 | 페이지/태스크 관리 | PROJECT-PAGE-003 | 페이지 삭제 | 프로젝트 하위 페이지를 확인창 후 `project_subtasks` 테이블에서 삭제한다. 연결 데이터 제약이 있으면 Supabase delete 오류를 표시한다. | 페이지 ID. | 삭제 성공/실패 메시지, 행 제거. | `src/pages/projects/ProjectEditorPage.tsx`, `src/api/projects.ts` | 구현됨 | 구현됨 | 권한별 삭제 버튼 노출, 삭제 성공/실패 확인 |
| 조직 관리 | `/org/summary` | 업무보고 현황 | ORG-SUMMARY-001 | 월별 사용자 업무 현황 조회 | admin이 `get_resource_summary` RPC 결과로 월별 사용자 업무 입력 시간과 부족/초과 상태를 확인한다. | 기준 월, admin이면 `p_member_id=null`, 일반 사용자는 본인 ID. | 사용자별 요약, 부족 시간, 상세 캘린더. | `src/pages/resource/ResourceSummaryPage.tsx`, `src/api/resources` | 구현됨 | 구현됨 | 월 변경, 사용자 필터, 부족 시간 계산 확인 |
...
| 조직 관리 | `/org/search` | 업무보고 관리 | ADMIN-REPORT-004 | 업무보고 삭제 | admin이 검색 결과의 업무보고를 `admin_delete_task` RPC로 삭제한다. 화면의 admin 라우트 가드 외 기간 제한은 없다. | task ID 필수, admin 라우트 접근 권한. | 삭제 후 목록 갱신, 실패 오류. | `src/pages/admin/reports/AdminReportsPage.tsx`, `src/api/tasks.ts` | 구현됨 | 구현됨 | 삭제 성공/실패, 일반 사용자 접근 차단 확인 |
| 조직 관리 | `/org/search/new`, `/org/search/:taskId/edit` | 업무보고 편집 | ADMIN-REPORT-EDIT-001 | 업무보고 대리 등록/수정 | admin이 대상 사용자를 선택해 신규 업무를 등록하거나 기존 업무를 `admin_save_task` RPC로 수정한다. 화면의 admin 라우트 가드 외 기간 제한은 없다. | 대상 사용자, 날짜, 타입, 프로젝트/페이지, 시간, 내용. | 저장 후 `/org/search` 또는 이전 검색 조건으로 이동. | `src/pages/admin/reports/AdminReportEditorPage.tsx`, `src/api/tasks.ts` | 구현됨 | 구현됨 | 신규/수정, 대상 사용자 변경, 필수값 오류 확인 |
...
| 리소스 | `/resource/month`, `/resource/month/:type` | 월간 리포트 | RESOURCE-MONTH-001 | 월간 리포트 조회 | admin이 기준 월의 총 MM, 휴무, 버퍼, 사용자별 초과/부족, 타입/서비스/보고서 탭 데이터를 확인한다. 부족/초과 기준 시간은 현재 월이면 오늘까지의 평일 수, 과거/미래 월이면 해당 월 전체 평일 수에 480분을 곱해 계산한다. | 기준 월, type route param. | 월간 요약, 표, 차트, 이전/다음 월 이동. | `src/pages/resource/ResourceMonthPage.tsx`, `src/pages/resource/ResourceMonthContextSection.tsx` | 구현됨 | 구현됨 | 월 이동, 탭 이동, route param, 평일×480분 산식 확인 |
...
| 태스크 현황 | `/stats/monitoring` | 모니터링 통계 | STATS-MON-003 | 태스크 상태 수정 | 모니터링 결과 행에서 추적 상태를 inline 수정한다. | 태스크/페이지 ID, 상태값. | 저장 후 행 상태 갱신, 실패 시 오류. | `src/pages/stats/TaskMonitoringResultsTable.tsx`, `src/api/stats` | 구현됨 | 구현됨 | 상태 변경 성공/실패 확인 |
...
| 관리자 | `/admin/platform/new`, `/admin/platform/:platformId/edit` | 플랫폼 관리 | ADMIN-PLATFORM-002 | 플랫폼 저장/삭제/전환 | admin이 플랫폼명을 신규 생성·수정·삭제한다. 삭제는 확인창 후 `deletePlatformAdmin`을 호출하고, 연결 데이터는 전환 다이얼로그에서 다른 노출 플랫폼으로 `replacePlatformUsage` 처리할 수 있다. | 플랫폼명 필수, 노출 여부, 전환 대상 플랫폼. | 저장/삭제 후 목록 반영, 전환 후 관련 query 갱신. | `src/pages/admin/platforms/AdminPlatformEditorPage.tsx` | 구현됨 | 구현됨 | 필수값, 저장, 삭제, 전환 확인 |
...
| 관리자 | `/admin/cost-group/new`, `/admin/cost-group/:costGroupId/edit` | 청구그룹 관리 | ADMIN-COST-002 | 청구그룹 저장/삭제/전환 | admin이 청구그룹명을 생성·수정·삭제한다. 삭제는 확인창 후 `deleteCostGroupAdmin`을 호출하고, 연결 데이터는 전환 다이얼로그에서 다른 활성 청구그룹으로 `replaceCostGroupUsage` 처리할 수 있다. | 청구그룹명 필수, 활성 여부, 전환 대상 청구그룹. | 저장/삭제 후 목록 반영, 전환 후 관련 query 갱신. | `src/pages/admin/cost-groups/AdminCostGroupEditorPage.tsx` | 구현됨 | 구현됨 | 필수값, 저장, 삭제, 전환 확인 |
...
| 관리자 | `/admin/group/new`, `/admin/group/:serviceGroupId/edit` | 서비스 그룹 관리 | ADMIN-GROUP-002 | 서비스그룹 저장/삭제/전환 | admin이 업무/프로젝트 분류에 사용하는 서비스그룹을 생성·수정·삭제한다. 사용 중인 프로젝트가 있으면 삭제 버튼을 비활성화하고, 연결 데이터는 전환 다이얼로그에서 다른 활성 서비스그룹으로 `replaceServiceGroupUsage` 처리할 수 있다. | 그룹명, 서비스명, 청구그룹, 활성 여부, 전환 대상 서비스그룹. | 저장/삭제 후 목록 반영, 전환 후 프로젝트/업무/리소스 query 갱신. | `src/pages/admin/groups/AdminServiceGroupEditorPage.tsx` | 구현됨 | 구현됨 | 신규/수정/삭제 차단/전환 확인 |
...
| 세부 기능 | 동작 | 필수 확인 |
| --- | --- | --- |
| 일자 선택 | 업무 날짜를 선택하고 저장 payload에 반영한다. | 오늘/과거/미래 날짜 저장 확인 |
...
| 페이지 행 삭제 | 하위 페이지를 삭제한다. | 저장 시 프로젝트 하위 페이지 목록 diff에 따라 삭제 대상 반영 |
...
| 검증 규칙 | 타입2는 선택된 타입1 하위 active 항목이어야 한다. 페이지는 선택 프로젝트에 속해야 한다. 서비스그룹/프로젝트 필요 여부는 `getTaskTypeUiRule` 결과를 따른다. |
...
| 검증 규칙 | 페이지명 필수. 프로젝트 ID가 있어야 저장 가능. |
...
| 검증 규칙 | 대상 사용자 필수, 날짜/타입/시간 필수, 타입-프로젝트 입력 규칙 준수. |
...
| 검증 규칙 | type1/type2 필수, 사용 중 삭제 시 대체 타입 필요. |
...
| 예외/오류 | 필수값 누락, 사용 중 타입 삭제 시 대체 타입 없음, API 실패. |
...
| 성공 결과 | 사용자 목록 반영, pending/active 상태에 따라 라우팅 결과 반영. |
...
| 코드 근거 | 보류성 정책 표가 아니라 실제 구현 파일/API/RPC 기준으로 동작이 확정되어 있는가 |
